...
class FlashcardRepository(private val api: SupabaseApi, private val db: AppDatabase) {
    ...
    suspend fun resolveDialogueWordMeaning(token: String, glossary: Map<String, String>): String {
        val normalized = normalizeDialogueToken(token)
        val glossaryMeaning = findInGlossary(normalized, glossary)
        if (!glossaryMeaning.isNullOrBlank()) return glossaryMeaning

        val cached = db.dialogueWordMeaningDao().get(normalized)
        if (!cached?.banglaMeaning.isNullOrBlank()) return cached!!.banglaMeaning

        // First use the already-synced local vocabulary cache.
        val localCandidates = dialogueCandidates(normalized)
        for (candidate in localCandidates) {
            val local = db.vocabDao().getByKoreanWord(candidate)
            if (local != null && local.bangla_meaning.isNotBlank()) {
                val meaning = local.bangla_meaning
                db.dialogueWordMeaningDao().upsert(
                    DialogueWordMeaningEntity(normalized, meaning, "supabase_cache", System.currentTimeMillis())
                )
                return meaning
            }
        }

        // Then ask Supabase directly. This also works when the user has not synced recently.
        for (candidate in localCandidates) {
            try {
                val rows = api.getVocabWordExact("eq.${candidate}")
                val meaning = rows.firstOrNull()?.bangla_meaning
                if (!meaning.isNullOrBlank()) {
                    db.dialogueWordMeaningDao().upsert(
                        DialogueWordMeaningEntity(normalized, meaning, "supabase", System.currentTimeMillis())
                    )
                    return meaning
                }
            } catch (_: Exception) {
                // If Supabase is unavailable, continue to Gemini fallback.
            }
        }

        // Finally use Gemini only for the missing word and cache the result locally.
        val meaning = DialogueAiHelper.translateSingleKoreanWord(normalized)
        db.dialogueWordMeaningDao().upsert(
            DialogueWordMeaningEntity(normalized, meaning, "gemini", System.currentTimeMillis())
        )
        return meaning
    }

    private fun normalizeDialogueToken(token: String): String =
        token.trim().trim('.', ',', '?', '!', '"', '\'', '“', '”', '‘', '’', '(', ')', '[', ']', '。', '！', '？', '，')

    private fun findInGlossary(word: String, glossary: Map<String, String>): String? {
        val direct = glossary[word] ?: glossary.entries.firstOrNull { normalizeDialogueToken(it.key) == word }?.value
        if (!direct.isNullOrBlank()) return direct
        for (candidate in dialogueCandidates(word).drop(1)) {
            val value = glossary.entries.firstOrNull { normalizeDialogueToken(it.key) == candidate }?.value
            if (!value.isNullOrBlank()) return value
        }
        return null
    }

    private fun dialogueCandidates(word: String): List<String> {
        val result = linkedSetOf(word)
        // Common standalone Korean case/topic/object/locative endings.
        listOf("에서", "으로", "에게", "부터", "까지", "처럼", "보다", "만큼", "에는", "에서", "에", "은", "는", "이", "가", "을", "를", "도", "만", "의", "와", "과", "로").forEach { suffix ->
            if (word.length > suffix.length + 1 && word.endsWith(suffix)) {
                result.add(word.removeSuffix(suffix))
            }
        }
